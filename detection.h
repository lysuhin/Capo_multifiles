...
class Detection{
    ...
    private:
        ...
        void filterPoints(const vector <Point2d_counts>& points_in, vector <Point2d>& points_out, size_t threshold){
        ...
        void getFirstPoint(const vector <Point2d>& points, Point2d& first){
        ...
        void getFirstRow(const vector <Point2d>& points_in, vector <Point2d>& points_out){
            Point2d upper_point;
    ...
    public:
        ...
        void detect(Mat& image,
            ...
            if (this->needs_filtration)
                image = calcGradient(applyFiltering(image));//.clone();
            ...
            if (points_with_counts.size() > 0)
            {
                if (needs_points_filtering)
                {
                    filterPoints(points_with_counts, this->points_result, DETECTION_THRESHOLD);
                    if (verbose) cout << "number of points after filtering : " << this->points_result.size() << endl;
                }
                else
                {
                    filterPoints(points_with_counts, this->points_result, 1);
                }
                if (needs_drawing) drawDetected(image, this->points_result, 1);
                if (needs_upper)
                {
                    vector <Point2d> points_first_row;
                    getFirstRow(this->points_result, points_first_row);
                    if (needs_drawing) drawDetected(image, points_first_row, 2);
                    this->points_result.resize(points_first_row.size());
                    copy(points_first_row.begin(), points_first_row.end(), this->points_result.begin());
                    if (verbose) cout << "number of points in first row : " << points_result.size() << endl;
                }
            }
